...
class Quaternion {
	...
	public function copy(_quaternion:Quaternion):Quaternion {
		set_xyzw(_quaternion.x, _quaternion.y, _quaternion.z, _quaternion.w);
	...
	public function setFromEuler(_euler:Vector4, _order:ComponentOrder = XYZ):Quaternion {
	...
	public function setFromAxisAngle(_axis:Vector4, _angle:Float):Quaternion {
		...
		set_xyzw(_axis.x * _s, _axis.y * _s, _axis.z * _s, Math.cos(_halfAngle));
	...
	public function multiplyQuaternions(_a:Quaternion, _b:Quaternion):Quaternion {
		...
		set_xyzw(qax * qbw + qaw * qbx + qay * qbz - qaz * qby, qay * qbw + qaw * qby + qaz * qbx - qax * qbz, qaz * qbw + qaw * qbz + qax * qby - qay * qbx,
	...
	public function slerp(_qb:Quaternion, _t:Float):Quaternion {
		...
		if (Math.abs(sinHalfTheta) < 0.001) {
			set_xyzw(0.5 * (_x + x), 0.5 * (_y + y), 0.5 * (_z + z), 0.5 * (_w + w));

			return this;
		}

		var ratioA = Math.sin((1 - _t) * halfTheta) / sinHalfTheta, ratioB = Math.sin(_t * halfTheta) / sinHalfTheta;

		set_xyzw(_x * ratioA + x * ratioB, _y * ratioA + y * ratioB, _z * ratioA + z * ratioB, _w * ratioA + w * ratioB);
