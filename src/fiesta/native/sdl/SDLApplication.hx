 ...
 *
 * Based on https://github.com/openfl/lime/blob/develop/project/src/backend/sdl/SDLApplication.cpp
 */
#if mac
@:headerCode('
...
class SDLApplication extends Application {
	...
	public function new() {
		...
		SDLApplication.currentApplication = this;
    ...
    public function onTimer(interval:Int, v:Dynamic):Int {
        ...
        var userevent:sdl.Event.UserEvent = sdl.Event.UserEvent.init();
        ...
        SDL.pushEvent(RawPointer.addressOf(event));
    ...
    public function waitEvent(event:sdl.Event):Int{
        ...
        while(true){
            ...
            switch (SDL.peepEvents(event, 1, SDL_GETEVENT,  SDL_FIRSTEVENT, SDL_LASTEVENT)){
                case -1: {
                    if (isBlocking) cpp.vm.Gc.exitGCFreeZone();
                    return 0;
                }
                case 1: {
                    if (isBlocking) cpp.vm.Gc.exitGCFreeZone();
                ...
                default: {
                    if (!isBlocking) cpp.vm.Gc.enterGCFreeZone();
                    isBlocking = true;
                    SDL.delay(1);
                }
            }
